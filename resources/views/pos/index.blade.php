...
<div class="row">
    ...
    <div class="col-sm-8 text-right">
        <button class="btn btn-link" data-toggle="modal" data-target="#newService">
            <i class="icon ion-md-add"></i> Nueva Cita
        </button>
    </div>
</div>
<table class="table table-striped">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{$engagement->adviser->name}}</td>
            <td>{{$engagement->service->name}}</td>
            <td>${{number_format($engagement->price, 2)}}</td>
            <td>
                <i class="icon ion-md-create btnEdit" data-id="{{$engagement->id}}"></i>
            </td>
        </tr>
        @endforeach
    </tbody>
